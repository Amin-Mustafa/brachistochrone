...
class SpaceScene(Scene):
        ...
        *mobs: Mobject,
        ...
        for mob in mobs:
            if not hasattr(mob, "body"):
                ...
                mob.spacescene = self
                mob.shape.filter = pymunk.ShapeFilter(categories = layer, mask = collide_with)
    ...
    def make_static_body(
        self, *mobs: Mobject, elasticity: float = 0, friction: float = 0.8, layer:int = 0xFFFFFFFF, collide_with: int = 0xFFFFFFFF
    ) -> None:
        ...
        for mob in mobs:
            if isinstance(mob, (VGroup, Group)):
                self.make_static_body(*mob, elasticity=elasticity, friction=friction, layer=layer, collide_with=collide_with)
                continue
